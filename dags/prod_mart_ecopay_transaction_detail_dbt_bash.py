...
@dag(
    ...
    max_active_runs=1,
    schedule=CronTriggerTimetable("0 5 * * *", timezone="Asia/Ho_Chi_Minh"),
...
def prod_mart_ecopay_transaction_detail_dbt_bash():
    ...
    bash_cmd_run = (
        f"source {VENV_PATH} && dbt deps --project-dir {DBT_PROJECT_PATH} && "
        f"dbt run --project-dir {DBT_PROJECT_PATH} --select +models/mart/ecopay/ "
        f"--vars '{var_string}' --profiles-dir {profiles_path} --target prod --threads {threads}"
...
prod_mart_ecopay_transaction_detail_dbt_bash()
